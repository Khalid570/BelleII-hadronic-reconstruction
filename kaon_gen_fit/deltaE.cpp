void deltaE()
{
  ...
  TFile *input = new TFile("/home/belle2/khalid/Root_files/kaon_gen/kaongen_Mbc_fit.root");
  ...
  RooRealVar no_sig("no_sig", "no_sig", 3131.0, 1000.0, 10000.0);
  ...
  RooRealVar no_sig_pi("no_sig_pi", "no_sig_pi", 2419.0, 1000.0, 10000.0);
  ...
  RooChebychev bkg("bkg", "background p.d.f.", deltaE, RooArgList(c0, c1, c2));
  RooRealVar no_bkg("no_bkg","no_bkg", 1264.0, 0., 10000.0);

  //model
  RooAddPdf model("model","model", RooArgList(sig,sig_pi,bkg), RooArgList(no_sig,no_sig_pi,no_bkg));
  model.fitTo(data);
  ...
  mean_txt->DrawLatex(-0.09, 350, "#scale[0.8]{#color[6]{mean_{K} = -0.0002973 GeV}}");
  mean_txt->Draw();
  TLatex* sigma_txt = new TLatex();
  sigma_txt->DrawLatex(-0.09, 300, "#scale[0.8]{#color[6]{#sigma_{K} = 0.007876 GeV}}");
  sigma_txt->Draw();
  TLatex* mean_pi_txt = new TLatex();
  mean_pi_txt->DrawLatex(-0.09, 250, "#scale[0.8]{#color[6]{mean_{#pi} = 0.04604 GeV}}");
  mean_pi_txt->Draw();
  TLatex* sigma_pi_txt = new TLatex();
  sigma_pi_txt->DrawLatex(-0.09, 200, "#scale[0.8]{#color[6]{#sigma_{#pi} = 0.009564 GeV}}");
  sigma_pi_txt->Draw();
  TLatex* yield_txt = new TLatex();
  yield_txt->DrawLatex(-0.09, 150, "#scale[0.8]{#color[6]{signal yield = 3132}}");
  yield_txt->Draw();
  TLatex* chi_txt = new TLatex();
  chi_txt->DrawLatex(-0.09, 100, "#scale[0.8]{#color[6]{#chi^{2} = 0.7072}}");
  chi_txt->Draw();
