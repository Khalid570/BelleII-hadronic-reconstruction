void D_M()
{
  ...
  TFile *input = new TFile("/home/belle2/khalid/Root_files/kaon_gen/kaongen.root");
  ...
  mean_txt->DrawLatex(1.81, 2000, "#scale[0.8]{#color[6]{mean = 1.869 GeV/C^{2}}}");
  mean_txt->Draw();
  TLatex* sigma_txt = new TLatex();
  sigma_txt->DrawLatex(1.81, 1800, "#scale[0.8]{#color[6]{#sigma = 0.003705 GeV/C^{2}}}");
  sigma_txt->Draw();
  TLatex* lower_txt = new TLatex();
  lower_txt->DrawLatex(1.81, 1600, "#scale[0.8]{#color[6]{lower cut = 1.858 GeV/C^{2}}}");
  lower_txt->Draw();
  TLatex* upper_txt = new TLatex();
  upper_txt->DrawLatex(1.81, 1400, "#scale[0.8]{#color[6]{upper cut = 1.881 GeV/C^{2}}}");
  upper_txt->Draw();
  TLatex* chi_txt = new TLatex();
  chi_txt->DrawLatex(1.81, 1200, "#scale[0.8]{#color[6]{#chi^{2} = 3.308}}");
  chi_txt->Draw();
  //TLatex* bkg_txt = new TLatex();
  //bkg_txt->DrawLatex(1.81, 1000, "#scale[0.5]{#color[6]{background = (0.6424) T_{0}(x) + (-0.6662) T_{1}(x) + (-0.5823) \\  T_{2}(x) + (-0.2283) T_{3}(x) + (0.2119) T_{4}(x)}}");
  //bkg_txt->Draw();
