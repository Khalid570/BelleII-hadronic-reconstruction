void Mbc()
{
  ...
  TFile *input = new TFile("/home/belle2/khalid/Root_files/pion_gen/piongen_D_M_fit.root");
  ...
  frame3->SetAxisRange(-1000., 1000.,"Y");
  ...
  mean_txt->DrawLatex(5.21, 14000, "#scale[0.8]{#color[6]{mean = 5.2794 GeV/C^{2}}}");
  mean_txt->Draw();
  TLatex* sigmaL_txt = new TLatex();
  sigmaL_txt->DrawLatex(5.21, 12000, "#scale[0.8]{#color[6]{#sigma_{L} = 0.002739 GeV/C^{2}}}");
  sigmaL_txt->Draw();
  TLatex* sigmaR_txt = new TLatex();
  sigmaR_txt->DrawLatex(5.21, 10000, "#scale[0.8]{#color[6]{#sigma_{R} = 0.003607 GeV/C^{2}}}");
  sigmaR_txt->Draw();
  TLatex* lower_txt = new TLatex();
  lower_txt->DrawLatex(5.21, 8000, "#scale[0.8]{#color[6]{lower cut = 5.2721 GeV/C^{2}}}");
  lower_txt->Draw();
  TLatex* upper_txt = new TLatex();
  upper_txt->DrawLatex(5.21, 6000, "#scale[0.8]{#color[6]{upper cut = 5.290 GeV/C^{2}}}");
  upper_txt->Draw();
  TLatex* chi_txt = new TLatex();
  chi_txt->DrawLatex(5.21, 4000, "#scale[0.8]{#color[6]{#chi^{2} = 10.22}}");
  chi_txt->Draw();

  canvas->cd();

  pad1->cd();
  TArrow *arrow1 = new TArrow(low_cut,2700,low_cut,2650,0.01);
