void D_M()
{
  ...
  TFile *input = new TFile("/home/belle2/khalid/Root_files/pion_gen/piongen.root");
  ...
  RooRealVar c6("c6", "coefficient #6", -0.1, -1.0, 1.0);
  ...
  frame3->SetAxisRange(-1500., 1500.,"Y");
  ...
  mean_txt->DrawLatex(1.81, 20000, "#scale[0.8]{#color[6]{mean = 1.869 GeV/C^{2}}}");
  mean_txt->Draw();
  TLatex* sigma_txt = new TLatex();
  sigma_txt->DrawLatex(1.81, 18000, "#scale[0.8]{#color[6]{#sigma = 0.003574 GeV/C^{2}}}");
  sigma_txt->Draw();
  TLatex* lower_txt = new TLatex();
  lower_txt->DrawLatex(1.81, 16000, "#scale[0.8]{#color[6]{lower cut = 1.859 GeV/C^{2}}}");
  lower_txt->Draw();
  TLatex* upper_txt = new TLatex();
  upper_txt->DrawLatex(1.81, 14000, "#scale[0.8]{#color[6]{upper cut = 1.880 GeV/C^{2}}}");
  upper_txt->Draw();
  TLatex* chi_txt = new TLatex();
  chi_txt->DrawLatex(1.81, 12000, "#scale[0.8]{#color[6]{#chi^{2} = 14.62}}");
  chi_txt->Draw();

  canvas1->cd();

  pad1->cd();
  TArrow *arrow1 = new TArrow(low_cut,1100,low_cut,1000,0.01);
  arrow1->SetLineWidth(2);
  arrow1->SetLineColor(kRed);
  arrow1->Draw();
  TArrow *arrow2 = new TArrow(high_cut,1100,high_cut,1000,0.01);
