void Mbc()
{
  ...
  RooDataSet data("data","dataset", tree, RooArgList(Mbc));

  //signal
  RooRealVar mean("mean","mean", 5.27949, 5.2, 5.29);
  RooRealVar sigma("sigma", "sigma", 0.00270166, 0.0001, 1);
  //RooRealVar sigmaR("sigmaR", "sigmaR", 0.00313585, 0.0001, 1);
  RooGaussian sig("sig","signal p.d.f.", Mbc, mean, sigma);
  ...
  RooChebychev bkg("bkg", "background p.d.f.", Mbc, RooArgList(c0, c1, c2, c3));
  ...
  mean_txt->DrawLatex(5.21, 3500, "#scale[0.8]{#color[6]{mean = 5.2796 GeV/C^{2}}}");
  mean_txt->Draw();
  TLatex* sigma_txt = new TLatex();
  sigma_txt->DrawLatex(5.21, 3000, "#scale[0.8]{#color[6]{#sigma = 0.002607 GeV/C^{2}}}");
  sigma_txt->Draw();
  TLatex* lower_txt = new TLatex();
  lower_txt->DrawLatex(5.21, 2500, "#scale[0.8]{#color[6]{lower cut = 5.272 GeV/C^{2}}}");
  lower_txt->Draw();
  TLatex* upper_txt = new TLatex();
  upper_txt->DrawLatex(5.21, 2000, "#scale[0.8]{#color[6]{upper cut = 5.287 GeV/C^{2}}}");
  upper_txt->Draw();
  TLatex* chi_txt = new TLatex();
  chi_txt->DrawLatex(5.21, 1500, "#scale[0.8]{#color[6]{#chi^{2} = 1.127}}");
  chi_txt->Draw();

  canvas->cd();

  pad1->cd();
  TArrow *arrow1 = new TArrow(low_cut,150,low_cut,100,0.01);
