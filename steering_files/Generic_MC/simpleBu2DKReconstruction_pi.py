...
x = 'pionID >0.5' + ' and ' + goodTrack
y = 'pionID >0.1' + ' and ' + goodTrack
...
ma.reconstructDecay(
    "anti-B0:DP -> D+:kpipi pi-:cuttt",
    cut="Mbc > 5.2 and abs(deltaE) < 0.5",
    path=main
)
vertex.treeFit("anti-B0:DP", massConstraint=["D+"], path=main)

# match reconstructed with MC particles
ma.matchMCTruth("anti-B0:DP", path=main)

# perform best candidate selection
b2.set_random_seed("m")
ma.rankByHighest("anti-B0:DP", variable='random', numBest=1, path=main)
...
fs_hadron_vars = vu.create_aliases_for_selected(
    ...
    decay_string='anti-B0 -> [D+ -> ^K- ^pi+ ^pi+] ^pi-',
