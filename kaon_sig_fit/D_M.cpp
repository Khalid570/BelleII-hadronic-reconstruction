void D_M()
{
  ...
  TFile *input = new TFile("~/sad15/sad/kaonsignal/kaonsig.root");
  TTree *dsttree = (TTree*)input->Get("dsttree");
  //TLatex Tl;

  RooRealVar D_M("D_M","D_M",1.8,1.9);
  RooDataSet data("data","dataset", dsttree, RooArgList(D_M));

  //signal
  RooRealVar mean("mean","mean",1.86944, 1.8, 1.9);
  RooRealVar sigma("sigma", "sigma", 0.00354838, 0.0001, 1);
  RooGaussian sig("sig","signal p.d.f.", D_M, mean, sigma);
  
  //bkg
  RooRealVar c0("c0", "coefficient #0",  0.737739, -1.0, 1.0);
  RooRealVar c1("c1", "coefficient #1", -0.782913, -1.0, 1.0);
  RooRealVar c2("c2", "coefficient #2", -0.840103, -1.0, 1.0);
  RooRealVar c3("c3", "coefficient #3", 0.131709, -1.0, 1.0);
  RooRealVar c4("c4", "coefficient #4", 0.440778, -1.0, 1.0);
  RooRealVar c5("c5", "coefficient #5", 0.263100, -1.0, 1.0);
  RooRealVar c6("c6", "coefficient #6", -0.496034, -1.0, 1.0);
  RooChebychev bkg("bkg", "background p.d.f.", D_M, RooArgList(c0, c1, c2, c3, c4, c5, c6));
  RooRealVar fsig("fsig","signal fraction", 0.828035, 0., 1.);
  ...
  frame2->GetYaxis()->SetNdivisions(702);
  frame2->SetAxisRange(-7., 7.,"Y");
  ...
  mean_txt->DrawLatex(1.81, 25000, "#scale[0.8]{#color[6]{mean = 1.869 GeV/C^{2}}}");
  mean_txt->Draw();
  TLatex* sigma_txt = new TLatex();
  sigma_txt->DrawLatex(1.81, 20000, "#scale[0.8]{#color[6]{#sigma = 0.003551 GeV/C^{2}}}");
  sigma_txt->Draw();
  TLatex* lower_txt = new TLatex();
  lower_txt->DrawLatex(1.81, 15000, "#scale[0.8]{#color[6]{lower cut = 1.859 GeV/C^{2}}}");
  lower_txt->Draw();
  TLatex* upper_txt = new TLatex();
  upper_txt->DrawLatex(1.81, 10000, "#scale[0.8]{#color[6]{upper cut = 1.880 GeV/C^{2}}}");
  upper_txt->Draw();
  TLatex* chi_txt = new TLatex();
  chi_txt->DrawLatex(1.81, 5000, "#scale[0.8]{#color[6]{#chi^{2} = 19.08}}");
  chi_txt->Draw();
  
  canvas1->cd();

  pad1->cd();
  TArrow *arrow1 = new TArrow(low_cut,1000,low_cut,900,0.01);
  arrow1->SetLineWidth(2);
  arrow1->SetLineColor(kRed);
  arrow1->Draw();
  TArrow *arrow2 = new TArrow(high_cut,1000,high_cut,900,0.01);
